...
def fill(grupo):
    ...
    cnn = sqlite3.connect('secundaria.db')
    c = cnn.cursor()

    c.executemany('''INSERT INTO estudiante (name,age,grade,tutor)
                VALUES(?,?,?,?); ''',grupo)
    cnn.commit()
    cnn.close()
...
if __name__ == '__main__':
    ...
    grupo = [('Marcos',31,5,'Juan'),
            ('Carlos',25,1,'Pepe'),
            ('Sebastian',28,3,'Fulano'),
            ('Raul',36,5,'Mengano'),
            ('Matias',41,2,'Ezequiel')]
